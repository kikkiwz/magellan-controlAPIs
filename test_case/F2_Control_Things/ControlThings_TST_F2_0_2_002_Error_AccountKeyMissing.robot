...
*** Test Cases ***
ControlThings_TST_F2_0_2_002_Error_AccountKeyMissing
    ...
    #Headers
	${headers}=    Create Dictionary        Content-Type=${HEADER_CONTENT_TYPE}    x-ais-UserName=${HEADER_X_AIS_USERNAME_AISPARTNER}    x-ais-OrderRef=${HEADER_X_AIS_ORDERREF_CREATECONTROLTHINGS}${current_timestamp}    x-ais-OrderDesc=${HEADER_X_AIS_ORDERDESC_CREATECONTROLAPI}    x-ais-AccessToken=Bearer ${accessToken}    Accept=${HEADER_ACCEPT}  
	...
	Log ControlThings      ${VALUE_RESULTCODE_40300}    ${VALUE_RESULTDESC_ACCOUNTKEY_MISSING_OR_INVALID}    ${res}    ${URL_CREATECONTROLTHINGS}    ${dataSearch}    ${accessToken}    ${AccountId}    ${ThingId}    ${VALUE_SENSORKEY_POINT}    ${sensorValue}    
