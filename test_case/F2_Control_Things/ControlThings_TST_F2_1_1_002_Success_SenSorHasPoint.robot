...
*** Test Cases ***
ControlThings_TST_F2_1_1_002_Success_SenSorHasPoint
    ...
    ${checkReponse}=    Run keyword And Continue On Failure    Response ResultCode Should Have    ${res}    ${CREATECONTROLTHINGS}    ${FIELD_OPERATIONSTATUS_LOWCASE}    ${FIELD_CODE_LOWCASE}    ${FIELD_DESCRIPTION_LOWCASE}
    Log To Console    checkReponse is : ${checkReponse}
	#====== Check log and Verify DB ==========
	${dataSearch}=    Set Variable    ${HEADER_X_AIS_ORDERREF_CREATECONTROLTHINGS}${current_timestamp}
	#Check log detail and summary
	Log ControlThings      ${VALUE_RESULTCODE_20000}    ${VALUE_DESCRIPTION_CREATECONTROLTHINGS_SUCCESS}    ${res}    ${URL_CREATECONTROLTHINGS}    ${dataSearch}    ${accessToken}    ${AccountId}    ${ThingId}    ${VALUE_SENSORKEY_POINT}    ${sensorValue} 
