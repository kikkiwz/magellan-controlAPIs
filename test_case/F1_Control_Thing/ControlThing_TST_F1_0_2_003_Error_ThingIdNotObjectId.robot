...
*** Test Cases ***
ControlThing_TST_F1_0_2_003_Error_ThingIdNotObjectId
	...
	#Body
    ${body}=    Evaluate    { "ThingId":"${ThingId_TST_F1_0_2_003}","Sensors":{"${VALUE_SENSORKEY}":"${sensorValue}"}}
    Log To Console    Body is : ${body}
    #Response
    ${res}=    Run keyword And Continue On Failure    Post Api Request    ${url}${CONTROLAPIS}    ${URL_CREATECONTROLTHING}    ${headers}    ${body}
	Log To Console    Response is : ${res}
    ${checkReponse}=    Run keyword And Continue On Failure    Response ResultCode Should Have Error    ${res}    ${VALUE_RESULTCODE_40300}    ${VALUE_RESULTDESC_THING_NOT_OBJECTID}
    Log To Console    checkReponse is : ${checkReponse}
	#====== Check log and Verify DB ==========
	${dataSearch}=    Set Variable    ${HEADER_X_AIS_ORDERREF_CREATECONTROLTHING}${current_timestamp}
	#Check log detail and summary
	Log ControlThing      ${VALUE_RESULTCODE_40300}    ${VALUE_RESULTDESC_THING_NOT_OBJECTID}    ${res}    ${URL_CREATECONTROLTHING}    ${dataSearch}    ${accessToken}    ${AccountId}    ${ThingId_TST_F1_0_2_003}    ${VALUE_SENSORKEY}    ${sensorValue}
